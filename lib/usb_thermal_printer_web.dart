...
class WebThermalPrinter {
  ...
  Future<void> pairDevice({
    ...
    if (!isAlreadyPaired!) {
      ...
      if (pairedDevice != null) {
        ApplicationData().pairedDevice = pairedDevice;
      }
    } else {
      pairedDevice = ApplicationData().pairedDevice;
    }

    print("pairedValueImmidiate: ${ApplicationData().pairedDevice}");
  ...
  Future<void> cutPaper() async {
    ...
    var encodedText = utf8.encode("\n");
    //  final cutCommand = Uint8List.fromList([0x1B, 0x64, 2]);
    var buffer = Uint8List.fromList([0x1D, 0x56, 0]).buffer;
    await usbDevice.transferOut(pairedDevice, endpointNumber, buffer);
  }
  ...
  Future<void> openDrawer() async {
    ...
    var encodedText = utf8.encode("\n");
    //  final cutCommand = Uint8List.fromList([0x1B, 0x64, 2]);
    var buffer = Uint8List.fromList([27,112,0,25,251]).buffer;
    await usbDevice.transferOut(pairedDevice, endpointNumber, buffer);
  }
